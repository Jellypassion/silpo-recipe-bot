...
застосунок Сільпо й оплатити замовлення.
...
- **`src/agents/cartAgent.ts`** — крок 2, виклик Claude з підключеним **нативним MCP-конектором**
  ...
  використовуючи OAuth access token поточного користувача.
...
5. Бот шукає товари через Silpo MCP, пріоритетно обирає акційні, додає в кошик, і присилає
   підсумок.
6. Користувач відкриває застосунок «Сільпо» і оплачує.
...
- Реальний виклик tool із `tools/list` у робочому сценарії — кожен запуск `cartAgent` викликає
  щонайменше `silpo_find_products_batch`/`silpo_add_or_update_cart_products` тощо.
...
- Явне узгодження кількості упаковок з користувачем перед додаванням у кошик.
